...
var pagesToVisit = jsonfile.readFileSync('tv-links.json');
var pagesUrls = [];
var numItems = 0;
var allData = [];
...
function scrapeItem($, url) {
    ...
    if (model.includes('LED')) {
        model = model.split("LED");
        item.model = model[1].trim().split(" ")[0].trim();
    }else{
        item.model="";
    }

    if (!item.model) {
        item.model = model[0].trim();
    }
